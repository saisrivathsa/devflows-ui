...
export default function buildState(response, resultsPerPage) {
  response.hits.hits.forEach(function(part, index) {
    var domain_id = response.hits.hits[index]["_source"]["domain_id"]
    var invocable_id = response.hits.hits[index]["_source"]["id"]
    response.hits.hits[index]["_source"]["id"] = domain_id + "-" + invocable_id;
    response.hits.hits[index]["_source"]["url"] = constants.UI_URL + "/domains/" + domain_id + "/invocables/" + invocable_id;    
  });
  const results = buildResults(response.hits.hits);
  
  results.forEach(function(part, index) {
    delete results[index]["domain_id"]
  })
