...
function buildMatch(searchTerm) {
  return searchTerm
    ? {
      simple_query_string: {
          query: searchTerm.indexOf('-') > -1 || searchTerm.indexOf('*') > -1 ? searchTerm : searchTerm + "*",
          fields: ["name", "description", "domain_name"]
        }
      }
    : { match_all: {} };
}
...
export default function buildRequest(state) {
  ...
  const body = {
    ...
    _source: ["name", "description", "domain_name", 'id', 'domain_id'],
    aggs: {
      domain_name: {
        terms: { field: "domain_name.keyword", size: 100 }
      }
    },
